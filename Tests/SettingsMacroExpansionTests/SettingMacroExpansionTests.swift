...
final class SettingMacroExpansionTests: XCTestCase {
    ...
    func testAllPropertyListTypes() throws {
        ...
        let allCases: [Case] = [
            ...
            Case(type: "Double", initializer: "3.14", description: "Double"),
            Case(type: "Float", initializer: "2.71", description: "Float"),
            Case(type: "String", initializer: "\"default\"", description: "String"),
            Case(type: "Date", initializer: "Date()", description: "Date"),
            Case(type: "Data", initializer: "Data()", description: "Data"),
            Case(type: "Array<Any>", initializer: "[]", description: "Array of Any"),
            Case(type: "Dictionary<String, Any>", initializer: "[:]", description: "Dictionary of Any"),
            ...
            Case(optionalType: "Array<Any>", description: "Optional Array of Any"),
            Case(optionalType: "Dictionary<String, Any>", description: "Optional Dictionary of Any"),
            
            // Collections
            Case(type: "[String]", initializer: "[]", description: "[String]"),
            Case(type: "Array<Int>", initializer: "[]", description: "Array<Int>"),
            Case(type: "[String: Int]", initializer: "[:]", description: "[String: Int]"),
        ]
...
extension SettingMacroExpansionTests {
    ...
    private func makeSource(
        ...
        let initializer = testCase.initialValue.isEmpty ? "" : " = \(testCase.initialValue)"
        let staticModifier = testCase.isStatic ? "static " : ""
        
        let (nestedContainerDecl, containerBaseName) = makeNestedContainerDecl(from: testCase)
    ...
    private func attributeTypeName(containerQualifiedName: String, property: String) -> String {
        let sanitizedContainer = containerQualifiedName.replacingOccurrences(of: ".", with: "_")
        return "__Attribute_\(sanitizedContainer)_\(property)"
    }
    
    private func makeNestedContainerDecl(from testCase: TestCase, forSource: Bool = true) -> (decl: String, container: String) {
        // Make nested container declaration:
        var nestedContainerDecl: String = ""
        let components = testCase.containerQualifiedName.split(separator: ".").map(String.init)
    ...
    private func makeExpectedExpandedSource(
        ...
        let (nestedContainerDecl, containerBaseName) = makeNestedContainerDecl(from: testCase, forSource: false)

        // make name decl
        let nestedNameComponents = testCase.containerQualifiedName.split(separator: ".").dropFirst(1).map(String.init) + ["setting"]
...
extension String.StringInterpolation {
    mutating func appendInterpolation(_ value: String, indentation: Int, prependNewLine: Bool = true) {
        ...
        let string = value.split(separator: "\n", omittingEmptySubsequences: false).joined(separator: "\n" + spaces)
